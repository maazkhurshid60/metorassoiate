    ...
    >
      <div className="container-x relative">
        ...
        <div className="mt-14 grid gap-px overflow-hidden border border-white/10 bg-white/10 md:grid-cols-2">
          {TESTIMONIALS.map((t) => (
            <figure key={t.name} className="flex flex-col bg-navy-900 p-8 sm:p-10">
              <div className="flex items-center justify-between">
                ...
                <span className="mono-label text-[9px] text-ink-500">★ VERIFIED</span>
              </div>
              <blockquote className="mt-5 flex-1 text-lg leading-8 text-ink-100 text-pretty">
                &ldquo;{t.quote}&rdquo;
              </blockquote>
              <figcaption className="mt-7 flex items-center gap-3 hair pt-5">
                <span className="inline-flex h-11 w-11 items-center justify-center border border-amber-500/40 text-sm font-bold text-amber-400">
                  {initials(t.name)}
                </span>
                <span>
                  <span className="block text-sm font-bold text-white">{t.name}</span>
                  <span className="mono-label block text-[9px] text-ink-400">
                    {t.role}
                  </span>
                </span>
              </figcaption>
