...
export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  ...
  const keywords = [
    ...
    `MEP staffing agency ${c.city} ${c.abbr}`,
    `mechanical electrical plumbing recruiter ${c.city}`,
    `HVAC engineer recruiter ${c.city}`,
    `MEP engineer jobs ${c.city}`,
  ];
  return {
    title,
    description,
...
export default async function MepCityPage({ params }: { params: Promise<{ city: string }> }) {
  ...
  const schemas = [
    ...
    breadcrumbSchema([
      { name: "Home", path: "/" },
      { name: "MEP Engineering Recruiter", path: "/mep-engineering-recruiter" },
      ...
      <section className="relative border-t border-navy-950/10 blueprint-light py-24 sm:py-28">
        <div className="container-x">
          <div className="max-w-2xl">
            ...
            <h2 className="display mt-5 text-4xl text-navy-950 sm:text-5xl">
              Supporting {c.city} building & infrastructure projects
            </h2>
          </div>
          ...
          <div className="container-x">
            <div className="grid gap-14 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
              <div>
                ...
                <h2 className="display mt-5 text-4xl text-navy-950 sm:text-5xl">
                  What MEP engineering in {c.city}{" "}
              ...
              {c.agencies && c.agencies.length > 0 && (
                <div className="border border-navy-950/10 bg-white p-8 sm:p-10">
                  <p className="mono-label text-slate-500">Owners & code authorities</p>
                  <p className="mt-3 text-sm leading-relaxed text-slate-500">
                    The owners and authorities behind most {c.city} MEP work. Whose projects a candidate has stamped, and under which code official, is the shorthand hiring managers here read first.
                  </p>
            ...
            {c.licensure && (
              <div className="corners mt-8 border border-navy-950/10 bg-white p-8 sm:p-10">
                <p className="mono-label text-slate-500">
                  Licensure & code authority in {c.state}
                </p>
